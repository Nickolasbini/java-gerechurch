...
public class Funcionarios extends JFrame {
	 ...
	 * Create the frame.
	 */
	public Funcionarios(Pessoa obj) {
		if(obj == null) {
		...
		btnRemove.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(id.equals(-1)) {
					...
					if(openModal("Tem certeza que deseja remover o(a) funcionário(a) "+workerName, "Confirmação de Exclusão") == true) {
						Boolean resultado = PessoaController.removerPorId(idToRemove);
						if(resultado.equals(true)) {
							displayMessage("Funcionário removido");
							currentFrame.setVisible(false);
		...
		btnNew.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				objetoNovo.setStatus(objetoNovo.FUNCIONARIO);
				objetoNovo.setInstitutionId(loggedUser.getInstitutionId());
				objetoNovo.setLgpd(true);
				SalvarPessoaModal saveModal = new SalvarPessoaModal(loggedUser);
	        	saveModal.setVisible(true);
	...
	public void listarFuncionarios() {
		dados   = funcionarioController.buscarMeusFuncionarios();
		colunas = funcionarioController.buscarColunasTabela();
		table = new JTable(dados, colunas);
		table.addMouseListener(new MouseAdapter() {
			@Override
		    public void mouseReleased(MouseEvent mouseEvent) {
		        JTable innerTable =(JTable) mouseEvent.getSource();
		        Point point = mouseEvent.getPoint();
		        int row = innerTable.rowAtPoint(point);
		        if (mouseEvent.getClickCount() == 2 && innerTable.getSelectedRow() != -1) {
		            // your valueChanged overridden method
		        	String idObjetoAlteravel = (String) innerTable.getModel().getValueAt(innerTable.getSelectedRow(), 0);
		        	SalvarPessoaModal saveModal = new SalvarPessoaModal(loggedUser);
		        	saveModal.setVisible(true);
		        	saveModal.setObjetoParaAlteracao(idObjetoAlteravel, null);
		        	saveModal.parentFrame = currentFrame;
	...
	public void restringirAcao() {
		if(loggedUser.getHasPrivilegies() == false) {
